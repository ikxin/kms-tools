...
export const AesKeyV4 = Buffer.from([
    0x05, 0x3D, 0x83, 0x07, 0xF9, 0xE5, 0xF0, 0x88,
    0xEB, 0x5E, 0xA6, 0x68, 0x6C, 0xF0, 0x37, 0xC7,
    0xE4, 0xEF, 0xD2, 0xD6
]);
...
export const AesKeyV6 = Buffer.from([
    0xA9, 0x4A, 0x41, 0x95, 0xE2, 0x01, 0x43, 0x2D,
...
function mixColumnsRBlock(buf: Buffer, off: number): void {
    for (let i = 0; i < AES_BLOCK_WORDS; i++) {
        ...
        const w = buf.readUInt32LE(byteOff);
        buf.writeUInt32LE(
            (mulE(w) ^ ror32(mulB(w), 8) ^ ror32(mulD(w), 16) ^ ror32(mul9(w), 24)) >>> 0,
            byteOff
        );
    }
}
...
function addRoundKey(buf: Buffer, blockOff: number, keyBuf: Buffer, keyByteOff: number): void {
    for (let i = 0; i < AES_BLOCK_BYTES; i++) {
    ...
    initKey(key: Buffer, isV6: boolean, keyBytes: number): void {
        ...
        for (let i = keyDwords; i < totalDwords; i++) {
            ...
            if ((i % keyDwords) === 0) {
                ...
                temp = be32(
                    (subDword(ror32(be32(temp), 24)) ^ RCon[rconIdx]) >>> 0
                );
            }
