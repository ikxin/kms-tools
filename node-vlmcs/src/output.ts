...
export function uuid2StringLE(buf: Buffer, offset = 0): string {
  ...
  return (
    ...
    d4[0].toString(16).padStart(2, '0') + d4[1].toString(16).padStart(2, '0') + '-' +
    Array.from(d4.subarray(2)).map(b => b.toString(16).padStart(2, '0')).join('')
  );
}

/** 在产品列表中按 GUID 查找名称 */
function findNameByGuid(guidStr: string, list: Array<{ guid: string; name: string }>): string {
 ...
 */
export function logResponseVerbose(ePID: string, hwid: Buffer, response: ParsedResponse, effectiveResponseSize: number): void {
