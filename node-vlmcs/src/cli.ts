...
import {
  guidToBuffer, stringToGuidLE,
  utf8ToUcs2, ucs2ToUtf8, unixTimeToFileTime,
  VERSION, PID_BUFFER_SIZE, WORKSTATION_NAME_BUFFER,
  ResponseResult,
} from './types';
import {
  createRequestV4, createRequestV6,
  decryptResponseV4, decryptResponseV6,
  ParsedResponse,
} from './kms';
...
import {
  logRequestVerbose, logResponseVerbose, uuid2StringLE,
  printPlatform, printCommonFlags, printClientFlags,
} from './output';
...
const DnsNames = {
  first: ['www', 'ftp', 'kms', 'hack-me', 'smtp', 'ns1', 'mx1', 'ns1', 'pop3', 'imap', 'mail', 'dns', 'headquarter', 'we-love', '_vlmcs._tcp', 'ceo-laptop'],
  second: ['.microsoft', '.apple', '.amazon', '.samsung', '.adobe', '.google', '.yahoo', '.facebook', '.ubuntu', '.oracle', '.borland', '.htc', '.acer', '.windows', '.linux', '.sony'],
  tld: ['.com', '.net', '.org', '.cn', '.co.uk', '.de', '.com.tw', '.us', '.fr', '.it', '.me', '.info', '.biz', '.co.jp', '.ua', '.at', '.es', '.pro', '.by', '.ru', '.pl', '.kr'],
};
...
interface ParsedOpt { opt: string; arg: string | null; }

function parseArgs(argv: string[]): { opts: ParsedOpt[]; positional: string[] } {
  ...
  while (i < argv.length) {
    const arg = argv[i];
    if (arg === '--') { i++; positional.push(...argv.slice(i)); break; }
...
function buildRequest(): Buffer {
  ...
  // WorkstationName
  if (workstationName) {
    const ws = utf8ToUcs2(workstationName.substring(0, 63), WORKSTATION_NAME_BUFFER);
    ws.copy(buf, 108);
  } else if (dnsnames) {
    const first = DnsNames.first[Math.floor(Math.random() * DnsNames.first.length)];
    const second = DnsNames.second[Math.floor(Math.random() * DnsNames.second.length)];
...
async function connectRpc(sock: net.Socket | null): Promise<{ sock: net.Socket }> {
  const s = await connectToAddress(remoteAddr, addressFamily, remoteAddr.startsWith('.') || remoteAddr === '-');
  if (verbose) process.stdout.write('\nPerforming RPC bind ...\n');
  const bindResult = await rpcBindClient(s, verbose, useClientRpcNDR64, useClientRpcBTFN, useMultiplexedRpc);
...
function displayResponse(
  ...
  if (!verbose) {
    ...
    if (response.majorVer > 5) {
      const hwidHex = Array.from(hwid.subarray(0, 8)).map(b => b.toString(16).toUpperCase().padStart(2, '0')).join('');
...
async function sendActivationRequest(
  sock: net.Socket, requestBase: Buffer
): Promise<{ status: number; response: ParsedResponse; result: ResponseResult; hwid: Buffer }> {
  let request: Buffer;
  ...
  const rpcResult = await rpcSendRequest(sock, request, rpcFlags, useClientRpcNDR64, firstPacketSent);
  ...
  const emptyResult: ResponseResult = {
    mask: 0, hashOK: false, timeStampOK: false, clientMachineIDOK: false,
    versionOK: false, ivsOK: false, decryptSuccess: false, hmacSha256OK: false,
    pidLengthOK: false, rpcOK: false, ivNotSuspicious: false,
    effectiveResponseSize: 0, correctResponseSize: 0,
  };
  const emptyResponse: ParsedResponse = {
    majorVer: 0, minorVer: 0, pidSize: 0, kmsPID: Buffer.alloc(0),
    cmid: Buffer.alloc(16), clientTime: Buffer.alloc(8), count: 0,
    vlActivationInterval: 0, vlRenewalInterval: 0,
  };
  return { status: rpcResult.status, response: emptyResponse, result: emptyResult, hwid: Buffer.alloc(8) };
}

/** 显示请求错误信息，参考 src/vlmcs.c 中的错误处理 */
function displayRequestError(status: number, currentRequest: number, totalRequests: number): void {
  errorout(`\nError 0x${(status >>> 0).toString(16).toUpperCase().padStart(8, '0')} while sending request ${currentRequest} of ${totalRequests}\n`);
  switch (status) {
    case 0xC004F042: errorout('The KMS server has declined to activate the requested product\n'); break;
    case 0x8007000D: errorout('The KMS host you are using is unable to handle your product. It only supports legacy versions\n'); break;
    case 0xC004F06C: errorout('The time stamp differs too much from the KMS server time\n'); break;
    case 0xC004D104: errorout('The security processor reported that invalid data was used\n'); break;
    case 1: errorout('An RPC protocol error has occured\n'); break;
  }
}
...
async function main(): Promise<void> {
  ...
  for (const o of opts) {
    if (o.opt === 'l' && o.arg) {
      ...
      if (num !== null) {
        ...
        if (idx < 0) {
          errorout(`Invalid client application. "${o.arg}" is not valid for -l.\n\n`);
          showProducts();
        }
  ...
  for (const o of opts) {
    switch (o.opt) {
      case 'h': case '?': showHelp(programName);
      case 'e': showExamples(programName);
      case 'x': showProducts();
      case 'V':
        process.stdout.write(`vlmcs ${VERSION} ${process.arch === 'x64' ? '64' : '32'}-bit\n`);
        printPlatform();
        printCommonFlags();
        printClientFlags();
        process.exit(0);
      case 'v': verbose = true; break;
      case 'm': vmInfo = true; break;
      case 'd': dnsnames = false; break;
      case 'T': reconnectForEachRequest = true; break;
      case 'P': break; // DNS SRV 优先级（未实现，仅接受参数）
      case 'p': useMultiplexedRpc = false; break;
      ...
      case 'i':
        if (o.arg) {
          ...
          else { errorout('IPv5 does not exist.\n'); process.exit(22); }
        }
        break;
      case 'n':
        incompatibleOptions |= VLMCS_OPTION_NO_GRAB_INI;
      ...
      case 'c':
        ...
        cmidStr = o.arg;
        break;
      case 'o': cmidPrevStr = o.arg; break;
      case 'a':
        incompatibleOptions |= VLMCS_OPTION_NO_GRAB_INI;
      ...
      case 'K':
        if (o.arg) {
          ...
          if (parts.length !== 2) { errorout('Fatal: Protocol version must be in the format #.#\n'); process.exit(22); }
      ...
      case 't':
        if (o.arg) {
          ...
          if (licenseStatus > 6) errorout('Warning: Correct license status is 0 <= license status <= 6.\n');
        }
        break;
      case 'g':
      ...
      case 'G':
        ...
        fnIniClient = o.arg;
        break;
      case 'l': case 'j': break; // Already handled
    }
  }

  if ((incompatibleOptions & (VLMCS_OPTION_NO_GRAB_INI | VLMCS_OPTION_GRAB_INI)) ===
...
async function runNormalMode(): Promise<void> {
  ...
  while (requestsToGo > 0) {
    ...
    const { status, response, result, hwid } = await sendActivationRequest(sock, requestBase);
    ...
    if (status) {
      ...
      if (!fixedRequests) {
        if (firstRequestSentLocal && nCountPolicy - response.count >= requestsToGo) {
...
async function grabServerData(): Promise<void> {
  ...
  for (let i = 0; i < KmsData.csvlkData.length && currentMajorVer > 3; i++) {
    ...
    for (let j = 0; j < KmsData.kmsItems.length; j++) {
      if (KmsData.kmsItems[j].ePidIndex === i) { kmsIdx = j; break; }
    }
    if (kmsIdx < 0) { lines.push(''); continue; }

    // Find a SKU item with this kms index
    let skuIdx = -1;
    for (let j = KmsData.skuItems.length - 1; j >= 0; j--) {
      if (KmsData.skuItems[j].kmsIndex === kmsIdx) { skuIdx = j; break; }
    }
    if (skuIdx < 0) { lines.push(''); continue; }
    ...
    try {
      ...
      if (status) {
        ...
        if (status === 1) break;
        if ((status & 0xF0000000) === 0x80000000) { currentMajorVer--; i--; }
        lines.push('');
        continue;
      }

      process.stdout.write(`${i + 7 - currentMajorVer} of ${KmsData.csvlkData.length + 6 - currentMajorVer}`);
      displayResponse(result, response, hwid);

      const ePID = ucs2ToUtf8(response.kmsPID, PID_BUFFER_SIZE);
      let line = `${ePidGroup} = ${ePID}`;
      if (response.majorVer > 5) {
        const hwidHex = Array.from(hwid.subarray(0, 8)).map(b => b.toString(16).padStart(2, '0').toUpperCase()).join(' ');
        line += ` / ${hwidHex}`;
      }
      lines.push(line);
    } catch (err: any) {
