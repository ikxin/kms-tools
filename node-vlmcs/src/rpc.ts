...
function packetTypeName(type: number): string {
  switch (type) {
    case RPC_PT_REQUEST: return 'Request';
    case RPC_PT_RESPONSE: return 'Response';
    case RPC_PT_FAULT: return 'Fault';
    case RPC_PT_BIND_REQ: return 'Bind';
    case RPC_PT_BIND_ACK: return 'Bind Ack';
    case RPC_PT_ALTERCONTEXT_REQ: return 'Alter Context';
    case RPC_PT_ALTERCONTEXT_ACK: return 'Alter Context Ack';
...
function parseBindResponse(
  ...
  if (verbose) {
    console.log(`  Max Xmit/Recv Frag: ${maxXmitFrag}/${maxRecvFrag}, AssocGroup: 0x${assocGroup.toString(16).padStart(8, '0')}`);
  ...
  for (let i = 0; i < numResults; i++) {
    ...
    if (verbose) {
      const resultStr = ackResult === RPC_BIND_ACCEPT ? 'Accept' :
        ackResult === RPC_BIND_NACK ? 'Nack' :
        ackResult === RPC_BIND_ACK ? 'Ack' : `Unknown(0x${ackResult.toString(16)})`;
      console.log(`  Result[${i}]: ${resultStr} (reason: 0x${ackReason.toString(16)})`);
    }
...
async function rpcBindOrAlterContext(
  ...
  if (isBind && useClientRpcBTFN) {
    ...
    ctxItems.push({ transferSyntax: BindTimeFeatureNegotiation, syntaxVersion: 1 });
  }

  const packetFlags = RPC_PF_FIRST | RPC_PF_LAST | (useMultiplexedRpc ? RPC_PF_MULTIPLEX : 0);
  const currentCallId = callId++;
  const bindPacket = buildBindPacket(packetType, packetFlags, currentCallId, ctxItems);
  ...
  if (verbose) {
    console.log(`Received RPC ${packetTypeName(header.packetType)} (FragLength=${header.fragLength}, CallId=${header.callId})`);
  }

  const expectedAckType = isBind ? RPC_PT_BIND_ACK : RPC_PT_ALTERCONTEXT_ACK;

  if (header.packetType === RPC_PT_FAULT || header.packetType !== expectedAckType) {
...
export async function rpcBindClient(
  ...
  const rpcFlags: RpcFlags = { hasNDR32: false, hasNDR64: false, hasBTFN: false };
  const rpcDiag: RpcDiag = { hasRpcDiag: false, hasBTFN: false, hasNDR64: false };

  // 第一步: 发送 Bind 请求
  let status = await rpcBindOrAlterContext(
    sock, RPC_PT_BIND_REQ, verbose,
    useClientRpcNDR64, useClientRpcBTFN, useMultiplexedRpc,
    rpcFlags, rpcDiag
  );
  ...
  if (!rpcFlags.hasNDR32) {
    status = await rpcBindOrAlterContext(
      sock, RPC_PT_ALTERCONTEXT_REQ, verbose,
      false, false, useMultiplexedRpc,
      rpcFlags, rpcDiag
    );
    if (status) return { status, rpcDiag, rpcFlags };
  }
...
): Promise<{ status: number; kmsResponse: Buffer | null; responseSize: number }> {
