...
// 重新导出时间工具函数
export { unixTimeToFileTime as getUnixTimeAsFileTime, fileTimeToUnixTime } from './types';
...
function emptyParsedResponse(): ParsedResponse {
  return {
    majorVer: 0, minorVer: 0, pidSize: 0,
    kmsPID: Buffer.alloc(0), cmid: Buffer.alloc(16), clientTime: Buffer.alloc(8),
    count: 0, vlActivationInterval: 0, vlRenewalInterval: 0,
  };
}
...
function createV6Hmac(
  ...
  hmacResult.copy(encryptStart, encryptSize - HMAC_SIZE, HALF_HASH_SIZE, HASH_SIZE);
}
...
export function decryptResponseV4(
  ...
  const postEpid = Buffer.from(rawResponse.subarray(copySize, copySize + V4_POST_EPID_SIZE));
...
export function decryptResponseV6(
  ...
  const result: ResponseResult = {
    mask: 0,
    hashOK: true, timeStampOK: true, clientMachineIDOK: true,
    versionOK: true, ivsOK: true, decryptSuccess: true,
    hmacSha256OK: true, pidLengthOK: true, rpcOK: true, ivNotSuspicious: true,
    effectiveResponseSize: totalSize, correctResponseSize: 0,
  };
  ...
  const postEpid = Buffer.from(response.subarray(copySize1, copySize1 + postEpidSize));

  const parsed = parseResponseBase(response, V6_UNENCRYPTED_SIZE, pidSize, postEpid);
  ...
  const randomXoredIVs = postEpid.subarray(V4_POST_EPID_SIZE, V4_POST_EPID_SIZE + 16);
  const receivedHash = postEpid.subarray(V4_POST_EPID_SIZE + 16, V4_POST_EPID_SIZE + 16 + HASH_SIZE);
  ...
  if (isV6) {
    ...
    const xoredIVs = postEpid.subarray(V5_POST_EPID_SIZE + 8, V5_POST_EPID_SIZE + 24);
    ...
    // 使用 ±1 时间槽容差验证 HMAC
    const savedHmac = Buffer.from(postEpid.subarray(V5_POST_EPID_SIZE + 24, V5_POST_EPID_SIZE + 40));
    ...
    for (let tolerance = -1; tolerance <= 1; tolerance++) {
      ...
      const computed = encryptStart.subarray(encryptSize - HMAC_SIZE, encryptSize);
